...
class BP_FileUpload
{
    ...
    protected $given_file_names = array();
    protected $db;
    protected $uid;

    function __construct(wpdb $wpdb, $uid, $options = null, $initialize = true, $error_messages = null) {
        $this->db = $wpdb;
        $this->uid = $uid;
        $this->options = array(
            ...
            'inline_file_types' => '/\.(gif|jpe?g|png|zip|mp4|doc?x|mp3|flv|avi|pdf)$/i',
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($file_size === $file->size) {
                ...
                $file_id = $this->save_files($file);
                $file->id = $file_id;
                $file->field_name = filter_input(INPUT_POST, 'field_name', FILTER_SANITIZE_STRING);
            } else {
                $file->size = $file_size;
    ...
    private function get_random_filename($file_name){
        $ext = strtolower(substr(strrchr($file_name, '.'), 1));
        $file_name = microtime() . '_' . rand(1, 9999);
    ...
    private function file_name_already_give($file_name)
    {
        foreach ($this->given_file_names as $value) {
            if ($file_name == $value) {
                return true;
            }
        }
    ...
    public function post($print_response = true) {
        ...
        $file_name = $this->get_random_filename($file_name);
        while($this->file_name_already_give($file_name)) {
            $file_name = $this->get_random_filename($file_name);
        }
        $this->given_file_names[] = $file_name;
    ...
    private function save_files($file)
    {
        ...
        $BP_File = new BP_File();
        $BP_File->path = '/upload/php/files/' . $file->name;
        $BP_File->mime_type = $file->type;
        $BP_File->size_bytes = $file->size;
        $BP_File->extension = $ext;
        $BP_File->is_image = $is_image;
        $BP_File->uid = $this->uid;
        
        $BP_Repo_Files = new BP_Repo_Files($this->db, biz_portal_get_table_prefix());
