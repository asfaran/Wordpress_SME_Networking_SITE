...
$ppolicy_post_id = get_option('policy_page_main');
$ppolicy_queried_post = get_post($ppolicy_post_id);

$first_side = get_option('policy_page_side_one');
    if($first_side['post']){
        $first_side_post = get_post($first_side['post']);           
    }

$second_side = get_option('policy_page_side_two');
    if($second_side['post']){
    $second_side_post = get_post($second_side['post']);
    }

$third_side = get_option('policy_page_side_three');
    if($third_side['post']){
...
?>
    ...
    <div class="page-container page-body devider_style_i">
		...
		<div class="col-md-8" style="border-right:#dedede 1px solid; text-align:left">
                    <!-- BEGIN TAB CONTENT -->
                    <h3 class="header_title_2"><?php echo $ppolicy_queried_post->post_title; ?></h3>
                    <div class="about_parag_left margin-bottom-20">
                        <?php
                        $content = apply_filters('the_content', $ppolicy_queried_post->post_content);
                        echo $content; ?>
                    </div>
                    <!-- END TAB CONTENT -->
		</div>   
