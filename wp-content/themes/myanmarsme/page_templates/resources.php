...
if (!is_user_logged_in()) {
    //BP_FlashMessage::Add('You should log into view this section', BP_FlashMessage::ERROR);
...
$current_user_can_edit_post = current_user_can('manage_options');
...
$slider_result = get_option('dashboard_directory_slider_resource');
if($slider_result['rmode']=='all'){
    $small_img_url= $slider_result['rimgsmall'];
    $big_img_url = $slider_result['rimgbig'];
}
?>
...
<style>
	...
	.top_banner_header{
		background-image:  url('<?php echo $big_img_url; ?>');
	}
	...
	@media(max-width:767px){
		.top_banner_header{
			background-image:  url('<?php echo $small_img_url; ?>');
		}
	}
</style>
    ...
    <div class="page-container page-body" style="font-size:13px">
        ...
        <div class="row">
        	<div class="container resources_search" >
					...
					if (data.hasOwnProperty('result')) {
						...
						if (data.result.hasOwnProperty('company')) {
							...
							if (data.result.company.id > 1) {
								...
								pm_link = company_link + '&send_message=true';
						...
						if (data.result.hasOwnProperty('profile')) {
							if(data.result.profile.hasOwnProperty('logo_id')) {
								if (data.result.profile.logo_id > 0) {
									new Image().src = file_download_url + '/?file_id=' + data.result.profile.logo_id + '&w=300&h=150';
								    var html = '<img src="' + file_download_url + '/?file_id=' + data.result.profile.logo_id + '&w=300&h=150' + '" />';
								    $("#model_company_logo").html(html);
								    $("#model_company_logo_mobile").html(html);	
								}
							}
							if (data.result.profile.hasOwnProperty('lightbox_image_id')) {
								if (data.result.profile.lightbox_image_id > 0) {
									sidebar_image_url = file_download_url + '/?file_id=' + data.result.profile.lightbox_image_id + '&default=1&w=300&h=300';
						...
						var html = '';
						html = '<img src="' + sidebar_image_url + '" class="img-responsive desktop_only">' +
      ...
      <div class="modal-dialog">
        <div class="modal-content" style="font-size:13px;">
                  	<div class="modal-header" id="modal_header">
                    		...
                    		<div id="model_company_logo_mobile"></div>
                	</div> 
                    ...
                    <div class="col-md-8">
                       <div class="row">
                       		<div class="text-center margin-top-20">
                            	<div id="model_company_logo" class="desktop_only"></div>
