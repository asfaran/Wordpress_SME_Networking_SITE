...
';
		...
		$string .= (self::$form_options['title_submit'] != '') ? esc_attr( self::$form_options['title_submit'] ) : esc_attr( __( 'Send', 'si-contact-form' ) );
		$string .= '" ';
        $onclick = 0;
...
$string .= "    </div>\n    <div " . self::get_this_css('field_div_style') . '>'
			...
			$string .= ' id="fscf_field'.self::$form_id_num.'_'.$key . '" name="message" cols="30" rows="4" ' . self::$aria_required;
	        if($field['attributes'] != '')
					  $string .= ' '.$field['attributes'];
             if ( 'true' == $field['placeholder'] && $field['default'] != '') {
                $string .= ' placeholder="'.esc_attr($field['default']).'"';
